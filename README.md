...
This package segments satellite imagery over large swaths of land (or sea).  The included examples use SpaceNet (link) labels to identify roads in high resolution satellite imagery.  Access to a GPU is required for training, though inference will function (albeit slowly) on a CPU.
